...
weights = [42, 69, 67, 57, 93, 90, 38, 36, 45, 42, 33, 79, 27, 57, 44, 84, 86, 92, 46, 38, 85, 33, 82, 73, 49, ...
    ...
    % Luego, ejecuta la función HHO con la función objetivo de Bin Packing
...
function fitness = bin_packing_objective(solution)
    global weights
    num_bins = max(solution);
    bins = zeros(1, num_bins);
    for i = 1:length(solution)
        bins(solution(i)) = bins(solution(i)) + weights(i);
    end
...
% ... (Resto del código de HHO) ...
    end
    toc
end

 [Rabbit_Energy,Rabbit_Location,CNVG] = HHO(N, T, lb, ub, dim, @bin_packing_objective);
